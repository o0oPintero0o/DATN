...
public class RequestServiceImpl extends BaseService implements RequestService {
    ...
    public MessageResponse addRequest(RequestRequest request) {
        Car car = roomRepository.findById(request.getCarId()).orElseThrow(() -> new BadRequestException("Thông tin phòng không tồn tại."));
        Request result = new Request(request.getNameOfRent(),request.getPhone(),request.getDescription(), car);
