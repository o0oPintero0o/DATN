...
public class MaintenanceServiceImpl extends BaseService implements MaintenanceService {
    ...
    public MessageResponse addNewMaintenance(String maintenanceDate, BigDecimal price, Long roomId, List<MultipartFile> files) {
        ...
        Maintenance maintenance = new Maintenance(LocalDateTime.parse(maintenanceDate),price,"http://localhost:8080/document/"+fileStorageService.storeFile(files.get(0)).replace("photographer/files/", ""),getUsername(), getUsername(), car);
